...
public class EntityBullet extends EntityArrow implements IProjectile, IThrowableEntity {
    ...
    public boolean haveSmokeTail = false;
    private float width = 0.5F, height = 0.5F;
    private String texture = "Bullet";
    
    public EntityBullet setTexture(String texture){ this.texture = texture; return this; }
    @Override
    ...
    public EntityBullet(World world, EntityLiving player, EntityLiving par3EntityLiving, float par4, float par5)
    {
        super(world);
        this.renderDistanceWeight = 10.0D;
    ...
    public EntityBullet(World world, EntityLiving player, float par3)
    {
        super(world);
        this.renderDistanceWeight = 10.0D;
        this.shootingEntity = player;
     ...
     */
    public void setThrowableHeading(double x, double y, double z, float speed, float vibrate)
    {
        float f2 = MathHelper.sqrt_double(x * x + y * y + z * z);
        x /= (double)f2;
        y /= (double)f2;
        z /= (double)f2;
        x += this.rand.nextGaussian() * (double)(this.rand.nextBoolean() ? -1 : 1) * 0.007499999832361937D * (double)vibrate;
        y += this.rand.nextGaussian() * (double)(this.rand.nextBoolean() ? -1 : 1) * 0.007499999832361937D * (double)vibrate;
        z += this.rand.nextGaussian() * (double)(this.rand.nextBoolean() ? -1 : 1) * 0.007499999832361937D * (double)vibrate;
        x *= (double)speed;
        y *= (double)speed;
        z *= (double)speed;
        this.motionX = x;
    ...
    public void onUpdate()
    {
        ...
        else
        {
            ...
            //this.setIsCritical(haveSmokeTail);
